 ...
 */
const getMealsWithMainIngredient = async (ingredient: string): Promise<Meal[]> => {
  ...
  const responseJson = (await response.json()) as MainIngredientResponse;
...
const getMealWithId = async (mealId: string): Promise<Meal | undefined> => {
  ...
  const responseJson: LookUpEndPointResponse = (await response.json()) as unknown as LookUpEndPointResponse;
...
app.get("/:mainIngredient", async (req, res) => {
  ...
  if (isIngredientPlural) {
    ...
    const mealsSingularIngredient: Meal[] = await getMealsWithMainIngredient(singularIngredient);
    meals = meals.concat(mealsSingularIngredient);
  } else {
    const pluralIngredient = mainIngredient + "s";
    const mealsPluralIngredient: Meal[] = await getMealsWithMainIngredient(pluralIngredient);
